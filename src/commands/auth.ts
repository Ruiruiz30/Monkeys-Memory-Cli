...
export async function login(args: CLIArgs): Promise<void> {
  ...
  if (!args.noOpen) await openBrowser(start.verification_uri_complete).catch(() => false);
  ...
  while (Date.now() < expiresAt) {
    const exchange = await apiResponse<{
    ...
    if (exchange.data.status === 'authorized' && exchange.data.token && exchange.data.token_id && exchange.data.expires_at) {
      ...
      const skills = await skillUpdateResult({ payload: {} }).catch((error) => ({ ok: false, error: (error as Error).message }));
...
export async function authStatus(): Promise<void> {
  ...
  const result = await apiRequest<JsonObject>('GET', '/api/v1/auth/me');
