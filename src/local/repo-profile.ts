...
function detectLanguages(paths: string[]): string[] {
  ...
  for (const filePath of paths) {
    ...
    else if (ext === '.md' || ext === '.mdx') languages.add('Markdown');
    else if (ext === '.css') languages.add('CSS');
    else if (ext === '.html') languages.add('HTML');
    else if (ext === '.py') languages.add('Python');
    else if (ext === '.tex') languages.add('LaTeX');
  }
  return uniqueSorted([...languages]);
}

function detectFrameworks(pkg: PackageJson | null, paths: string[]): string[] {
  const deps = { ...(pkg?.dependencies ?? {}), ...(pkg?.devDependencies ?? {}) };
  const frameworks = new Set<string>();
  if (deps.fastify) frameworks.add('Fastify');
  if (deps.express) frameworks.add('Express');
  if (deps.react) frameworks.add('React');
  if (deps.vite || paths.includes('vite.config.ts') || paths.includes('vite.config.js')) frameworks.add('Vite');
  if (deps.next || paths.includes('next.config.js') || paths.includes('next.config.mjs')) frameworks.add('Next.js');
  if (deps.typeorm) frameworks.add('TypeORM');
  if (deps.vitest) frameworks.add('Vitest');
  if (deps.typescript || paths.includes('tsconfig.json')) frameworks.add('TypeScript');
  if (paths.some(item => item.startsWith('skills/'))) frameworks.add('Codex/Claude Skills');
  return uniqueSorted([...frameworks]);
}

function inferKind(repoName: string, pkg: PackageJson | null, paths: string[], frameworks: string[]): string {
  const name = `${repoName} ${pkg?.name ?? ''}`.toLowerCase();
  if (pkg?.bin || name.includes('cli') || paths.some(item => item.startsWith('src/commands/'))) return 'cli-tool';
  if (
  ...
  if (paths.some(item => item.endsWith('.tex')) && !paths.some(item => item.startsWith('src/'))) return 'documentation';
  if (pkg?.name) return 'library';
  return 'repository';
}

function takePaths(paths: string[], predicate: (item: string) => boolean, limit = 12): string[] {
  return paths.filter(predicate).slice(0, limit);
}

function buildPathGroups(paths: string[]): RepoPathGroup[] {
  const groups: RepoPathGroup[] = [];
  const add = (name: string, purpose: string, items: string[]) => {
    if (items.length > 0) groups.push({ name, purpose, paths: uniqueSorted(items) });
  };
...
function commandPurpose(name: string): string {
  if (name.includes('test')) return 'Run automated tests.';
  if (name.includes('build')) return 'Build distributable output.';
  if (name.includes('check') || name.includes('lint')) return 'Run static validation.';
  if (name.includes('dev')) return 'Start local development mode.';
  if (name.includes('start')) return 'Start the application.';
  if (name.includes('worker')) return 'Run a background worker.';
  return 'Project script.';
}

function packageManagerRunner(pkg: PackageJson | null): string {
...
function importantCommands(pkg: PackageJson | null): Array<{ name: string; command: string; purpose: string }> {
...
function buildOwnership(kind: string, pathGroups: RepoPathGroup[]) {
  const owns = pathGroups.map(group => `${group.name}: ${group.paths.slice(0, 4).join(', ')}`);
  const doesNotOwn: string[] = [];
...
function scanLineForEntities(line: string): Array<{ kind: string; name: string; signature?: string }> {
  ...
  const entities: Array<{ kind: string; name: string; signature?: string }> = [];
  for (const [kind, regex] of matchers) {
    const match = trimmed.match(regex);
    if (match?.[1]) entities.push({ kind, name: match[1], signature: trimmed.slice(0, 160) });
  }
...
export async function extractCodeEntities(workspaceRoot: string, knownPaths: string[], limit = 5000): Promise<CodeEntity[]> {
  const entities: CodeEntity[] = [];
  for (const relativePath of knownPaths) {
    if (entities.length >= limit) break;
    if (!SOURCE_EXTENSIONS.has(fileExtension(relativePath))) continue;
    if (relativePath.split('/').some(part => SKIP_DIRS.has(part))) continue;
...
export async function buildRepoProfile(workspaceRoot: string, repoName: string, knownPaths: string[]): Promise<{ profile: RepoProfile; brief: string }> {
  const pkg = await readJson<PackageJson>(path.join(workspaceRoot, 'package.json'));
...
function renderRepoBrief(profile: RepoProfile): string {
  const lines = [
