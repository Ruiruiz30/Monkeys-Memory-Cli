...
function usage() {
  ...
  monkeys-memory agent-action-result --action-id <id> --type repo_scan [--repo <repo>] [--org-id <orgId>] [--workspace <dir>] [--data <json>] [--guide <text>] [--full-scan] [--no-report]
  monkeys-memory api <METHOD> <PATH> [--data <json>] [--params <json>] [--org-id <orgId>]`);
}
export async function main(argv) {
    await checkForCliUpdate().catch((error) => {
    ...
    if (command === 'login')
        return login(subArgs);
    if (command === 'logout')
        return logout();
    if (command === 'auth' && subArgs._[0] === 'status')
        return authStatus();
    if (command === 'config')
        return configCommand(subArgs);
    if (command === 'retrieve')
    ...
    if (command === 'update-skills' || command === 'install-skills')
        return updateSkills(subArgs);
    if (command === 'agent-capabilities') {
        console.log(JSON.stringify(await agentCapabilities(), null, 2));
        return;
    }
    if (command === 'agent-action-result')
        return agentActionResult(subArgs);
    if (command === 'api')
        return apiCommand(subArgs);
    usage();
    process.exitCode = 1;
}
